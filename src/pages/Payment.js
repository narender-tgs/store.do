...
const Payment = ({
    ...
    script.onload = () => {
      ...
      rzp1.on("payment.failed", (response) => {
        ...
        if (response?.error) {
          ...
          axios
            .post(
              `http://localhost:3000/v1/order/${orderGuid}/verify/payment`,
              orderBody,
              { headers: { service_ref: "8xuf4dev", paymentSignature: "" } },
            )
            .then((response) => {
  ...
  function paymentResponseHandler(response) {
    ...
    if (resOrderId) {
      const rzoPaySignature = response?.razorpay_signature;
      let orderBody = {
        order: {
        ...
        .post(
          ...
          orderBody,
          {
            headers: {
              ...
              "x-razorpay-signature": rzoPaySignature,
            },
          },
        )
        .then((response) => {
